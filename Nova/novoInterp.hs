...
-- eval env clo@(List (Atom "make-closure":(Atom "lambda":(List formals):body:[]))) = return clo

-- fazer lookup "let" em env, buscar var em formals; usar >>=
-- return (List (Atom "make-closure":(Atom "lambda":(List (formals ++ evalBack env 0 (size env))):body:[])))  
...
eval env lis@(List [Atom "make-closure2", lambda]) = 
  ST (\s e -> let
...
filterComment :: LispVal -> LispVal
filterComment (List x) = List ( Prelude.map filterComment $ Prelude.filter isComment x )
filterComment ls       = ls

isComment :: LispVal -> Bool
isComment ( List [ (Atom "comment"), (String _ )] ) = False
isComment ls = True
